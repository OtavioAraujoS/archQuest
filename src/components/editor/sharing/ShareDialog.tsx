...
export function ShareDialog({ diagram, onClose }: Readonly<ShareDialogProps>) {
  const [changeInProgress, setChangeInProgress] = useState<SharingChange | null>(null)
  ...
  return (
    <ModalDialog title="Compartilhar diagrama" onClose={onClose} className="max-w-lg">
      <p className="text-muted-foreground mb-4 text-sm">
        Quem tiver o link vê a versão atual do diagrama, só para leitura e sem precisar de
        conta. Seu nome e seu e-mail não aparecem.
      </p>

      {!isInCloud && (
        <p className="text-sm">
          Este diagrama ainda está indo para a nuvem. Quando aparecer "Salvo na nuvem", você
          poderá publicá-lo.
        </p>
      )}

      {isInCloud && !diagram.publicSlug && (
        <Button onClick={() => changeSharing('publishing')} disabled={changeInProgress !== null}>
      ...
      {isInCloud && diagram.publicSlug && (
        <div className="flex flex-col gap-4">
          <PublishedLinkField publicUrl={publicDiagramUrl(diagram.publicSlug)} />
          <div className="flex items-center justify-between gap-4">
            <p className="text-muted-foreground text-xs">
              Despublicar desativa este link. Publicar de novo gera um link diferente.
            </p>
            <Button
              variant="outline"
            ...
            >
              {changeInProgress === 'unpublishing' ? 'Despublicando…' : 'Despublicar'}
            </Button>
          </div>
        </div>
      )}
